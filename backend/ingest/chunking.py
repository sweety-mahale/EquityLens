...
def chunk_markdown(
    ...
    *,
    ...
    """Chunk raw markdown text by page boundaries and token budget.

    Extracts page numbers from <!-- page: N --> markers.
    """
        ...
        for para in paragraphs:
            ...
            if para_tokens > CHUNK_MAX_TOKENS:
                ...
                for sub in sub_paras:
                    sub_tokens = tokenizer.count_tokens(sub)
                    if chunk_tokens + sub_tokens > CHUNK_MAX_TOKENS:
                        if chunk_text_parts:
                            ...
                            if max_chunks is not None and len(records) >= max_chunks:
                                return records
                        chunk_text_parts = [sub]
                        chunk_tokens = sub_tokens
                    else:
                        chunk_text_parts.append(sub)
                        chunk_tokens += sub_tokens
            elif chunk_tokens + para_tokens > CHUNK_MAX_TOKENS:
                if chunk_text_parts:
                    text = "\n".join(chunk_text_parts)
                    records.append(
                        ChunkRecord(
                    ...
                    if max_chunks is not None and len(records) >= max_chunks:
                        return records
                chunk_text_parts = [para]
                chunk_tokens = para_tokens
            else:
                chunk_text_parts.append(para)
                chunk_tokens += para_tokens
