...
def ingest_document(
    ...
    *,
    ...
    doc_ident = document.accession_number or f"{document.ticker} {document.financial_year or ''} {document.document_type or ''}".strip()
    
    if force and not dry_run:
        _delete_chunks(session, document.id)
    elif skip_existing and _document_has_chunks(session, document.id):
        print(f"Skipping existing chunks for {doc_ident}")
        return 0
    ...
    if document.accession_number:
        ...
        print(f"Chunking {doc_ident} from database markdown_content...")
    ...
    if not records:
        print(f"No chunks produced for {doc_ident}")
        return 0
    ...
    session.commit()
    print(f"  Wrote {len(records)} chunk(s) for {doc_ident}")
    return len(records)
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    target.add_argument("--ticker", help="Process filings for a specific company ticker")
    ...
    if args.all:
        ...
        with Session(engine) as session:
            documents = session.scalars(
                select(SourceDocument).where(SourceDocument.ticker == args.ticker.upper())
            ).all()
            if not documents:
                print(f"No documents found for ticker {args.ticker}")
                return
            for document in documents:
                doc_ident = document.accession_number or f"{document.ticker} {document.financial_year or ''} {document.document_type or ''}".strip()
                if (
