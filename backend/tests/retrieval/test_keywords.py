...
def _mock_parse_response(terms: list[str]) -> MagicMock:
    response = MagicMock()
    response.parsed = FtsKeywordExtraction(terms=terms)
    return response


@patch("app.retrieval.keywords._client")
def test_extract_fts_keywords_returns_llm_terms(mock_client: MagicMock) -> None:
    mock_client.return_value.models.generate_content.return_value = _mock_parse_response(
    ...
    assert "Ticker filters: NVDA" in user_message
